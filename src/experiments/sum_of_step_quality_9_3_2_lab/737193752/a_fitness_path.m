function data = a_fitness_path( ...
    ...
    step_quality_sums = zeros(1, pop_size);

    if draw
       [im, map2draw] = draw_map(map, cmap, robot_bodies, sensor_lines, ...
           start_positions, target_positions,[], save_gif, gif_name);
    ...
    for step = 1:step_count
       ...
       if draw
            [im, map2draw] = draw_map(map, cmap, robot_bodies, sensor_lines, ...
                start_positions, target_positions, collis, save_gif, gif_name);
            pause(draw_refresh_rate);
       end

       angle_errors = get_angle_errors(robot_positions, robot_angles, target_positions);
       norm_angle_errors = normalize_angles(angle_errors);
       old_distances = target_distances;
       target_distances = euc_dist_3d(robot_positions, target_positions);
       norm_target_distances = normalize_distance(target_distances, max_distance);

       % tracking for log
       collisions = collisions + collis;
       % calc quality of step
       step_lens = old_distances - target_distances;
       ideal_step_lens = zeros(1, pop_size) + robot.maxSpeed;
       dist_lt_maxSpeed_idx = old_distances < robot.maxSpeed;
       ideal_step_lens(dist_lt_maxSpeed_idx) = target_distances(dist_lt_maxSpeed_idx);
       step_qualities = 1 - (step_lens ./ ideal_step_lens);
       collis_idx = collis > 0;
       step_qualities(collis_idx) = 20;
       
       
       step_quality_sums = step_quality_sums + step_qualities;   
