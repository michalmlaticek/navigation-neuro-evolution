...
clear
%rng_id = 5;
exp_folder = 'dist_collision10_angleerr001_net_9_3_2_pop_200';
...
experiment = sprintf('%s/%s', exp_folder, run_id);
draw = false;
...
save(sprintf('%s/settings', log_folder), 'settings')
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for gen = 1:settings.gen_count
    ...
    [Fit, dists, collis, angles] = fitness(Pop, settings.map, settings.netLayout, ...
    ...
    logger.debug(sprintf('%d ', collis));
    logger.debug('Distances:');
    logger.debug(sprintf('%f ', dists));
    logger.debug('Angles:');
    logger.debug(sprintf('%f', angles));
    ...
    Old=seltourn(Pop,Fit',25);
    Work1=selsus(Pop,Fit',30);
    Work2=seltourn(Pop,Fit',140);
