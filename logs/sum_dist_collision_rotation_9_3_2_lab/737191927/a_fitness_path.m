function data = a_fitness_path( ...
    ...
    goal_rotation = zeros(1, pop_size);

    if draw
       [im, map2draw] = draw_map(map, cmap, robot_bodies, sensor_lines, start_positions, target_positions,[], []);
    ...
    for step = 1:step_count
       ...
       if draw
            [im, map2draw] = draw_map(map, cmap, robot_bodies, sensor_lines, start_positions, target_positions, collis, []);
            pause(draw_refresh_rate);
       end
       ...
       close_proximity_idx = target_distances < 1 & target_distances > 0;
       goal_rotation(close_proximity_idx) = goal_rotation(close_proximity_idx) + abs(angle_errors(close_proximity_idx));
       exact_proximity_idx = target_distances == 0;
       goal_rotation(exact_proximity_idx) = goal_rotation(exact_proximity_idx) + abs(d_angles(exact_proximity_idx));
    ...
    data.goal_rotation = goal_rotation;
    
    %%%% Fitness %%%%%%%
    data.fits = data.dist_sum + 100*data.collisions + goal_rotation;
end
