function data = a_fitness_path( ...
    ...
    if draw
       [im, map2draw] = draw_map(map, cmap, robot_bodies, sensor_lines, start_positions, target_positions,[], save_gif, gif_name);
    ...
    for step = 1:step_count
       ...
       if draw
            [im, map2draw] = draw_map(map, cmap, robot_bodies, sensor_lines, start_positions, target_positions, collis, save_gif, gif_name);
            pause(draw_refresh_rate);
       end
    ...
    %%%% Fitness %%%%%%%
    data.fits = data.dist_sum + 1000*data.collisions;
end
