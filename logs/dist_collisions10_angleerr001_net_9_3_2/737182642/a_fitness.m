function [fits, target_distances, collisions, combined_angle_errs] = a_fitness(...
    weights, ...
    map, ...
    net_layout, ...
    robot, ...
...
if draw
   [im, map2draw] = draw_map(map, cmap, robot_bodies, sensor_lines, start_positions, target_positions);
       ...
       imwrite(map2draw,cmap,'simulation.gif','gif', 'Loopcount',inf);
...
for step = 1:step_count
   ...
   if draw
        ...
        % Write to the GIF File         
        imwrite(map2draw,cmap,'simulation.gif','gif','WriteMode','append');
...
function [colision_indicators, target_indicators] = get_collisions_and_targets(map, bodies, positions, target)
    ...
    for i = 1:size(positions, 2)
        %if is_collision(bodies, map)  % here was this error where the body
        %index was missing
...
function fitnesses = calc_fitnesses(distances, collisions, combined_angle_errs)
    fitnesses = distances + collisions*10 + combined_angle_errs*0.01;
end
