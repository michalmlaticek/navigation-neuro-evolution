function [fits, target_distances, collisions, combined_angle_errs] = fitness_dist_collisions100_angleerr01(weights, ...
    map, ...
    net_layout, ...
    robot, ...
    body, ...
...
for step = 1:step_count
   ...
   target_distances = euc_dist_3d(robot_positions, target_positions);
   norm_target_distances = normalize_distance(target_distances, max_distance);
   sensor_lines = get_sensor_lines(sensor_angles, robot_positions, robot);[collis, targets] = get_collisions_and_targets(map, robot_bodies, robot_positions, target_positions);
